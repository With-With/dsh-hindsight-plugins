...
- **主动同步开关**：区分两种同步方式
  - **主动同步（开关开启，默认）**：AI 每轮答完后自动写入 Hindsight（官方 `retainSessions` 机制，服务端折叠去重，不浪费提取）
  - **不主动同步（开关关闭）**：停止自动写入；对话中明确要求记住的内容（"把这个存进记忆"）依然正常入库--被动同步不受影响
  - 映射官方 `harnesses.dsh.retainSessions` 配置，对新会话生效
