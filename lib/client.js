...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const PROJECT_URL = "https://github.com/With-With/dsh-hindsight-plugins";
		...
		const CSS = [
			...
			".dhs-head-actions{margin-left:auto;display:flex;align-items:center;gap:8px}",
			...
			".dhs-banner-warn{border-color:rgba(210,153,34,.45);background:rgba(210,153,34,.08)}",
			...
			".dhs-help{position:relative;display:inline-flex}",
		...
		function adapterOk(data) {
			...
			if (adapter.duplicated) return false;
			if (typeof adapter.entryCount === "number") {
				return !!(adapter.runtimePresent && adapter.entryCount >= 1);
			}
		...
		/**「说明」button + popover: install command + docs link + notes. */
		function HelpButton() {
			...
			return h("div", { className: "dhs-help" },
				...
				open ? h("div", { className: "dhs-pop" },
					...
					h("div", null, h("a", {
						...
						target: "_blank",
						rel: "noreferrer",
					}, "本项目 GitHub：With-With/dsh-hindsight-plugins ->")),
					h("div", { className: "dhs-muted" },
						"安装或切换配置后，对新会话生效；本插件只负责配置与安装，官方运行时负责记忆。")) : null);
		}

		/**
		 * Warning banner: missing adapter (red, one-click install) or
		 * DUPLICATED registrations (amber, one-click convergence). The
		 * duplicate state breaks `dsh web` startup (duplicate loader entry
		 * id), so it deserves equal prominence.
		 */
		function AdapterWarning({ install, busy, onInstall, onDedupe }) {
			const phase = install && install.phase;
			const duplicate = phase === "duplicate";
			return h("div", { className: duplicate ? "dhs-banner dhs-banner-warn" : "dhs-banner" },
				duplicate
					? h("div", { className: "dhs-err" },
						"⚠ 官方适配器重复注册（共 " + ((install && install.layers) || []).length
						+ " 层）-- 会导致下次 dsh web 启动失败（duplicate loader entry id）。")
					: h("div", { className: "dhs-err" }, "⚠ 外部记忆库当前不可用：官方 DSH 适配器未安装或未挂载。"),
				duplicate && install && install.layers && install.layers.length
					? h("div", { className: "dhs-muted" },
						install.layers.map((layer) => layer.count + "x " + layer.file).join("  ·  "))
					: null,
				phase === 'running'
				...
				install && install.log && install.log.length
					...
					: null,
				duplicate ? null : h("div", { className: "dhs-muted" }, "或手动在终端执行："),
				duplicate ? null : h("div", { className: "dhs-code" }, INSTALL_CMD),
				duplicate ? null : h("div", null, docsLink("查看官方安装文档 ->")));
		}
		...
		function HindsightSettingsTab({ api }) {
			...
			const dedupeAdapter = async () => {
				setInstallBusy(true);
				try {
					const data = await api("/dedupe", { method: "POST", body: "{}" });
					setState((s) => (s.phase !== "ready" ? s : Object.assign({}, s, { data })));
				} catch (error) {
					const message = String((error && error.message) || error);
					setState((s) => (s.phase !== "ready" ? s : Object.assign({}, s, { error: message })));
			...
			const toggleRetain = async () => {
				...
				try {
					const saved = await api("/retain", {
						method: "POST",
						body: JSON.stringify({ enabled: !(data.retainSessions === true) }),
					});
					// No toast (v0.4.2): the switch state itself is the feedback.
					setState((s) => (s.phase !== "ready"
						? s
						: Object.assign({}, s, { data: saved, notice: null, error: null })));
				} catch (error) {
					const message = String((error && error.message) || error);
					setState((s) => (s.phase !== "ready" ? s : Object.assign({}, s, { error: message })));
				}
			...
			return h("div", { className: "dhs-root" },
				h("div", { className: "dhs-head" },
					...
					h("div", { className: "dhs-head-actions" },
						h(HelpButton),
						h("button", {
							className: "dhs-btn dhs-primary",
				...
				ok ? null : h(AdapterWarning, {
					install,
					busy: installBusy,
					onInstall: installAdapter,
					onDedupe: dedupeAdapter,
				}),
					...
					h("div", { className: "dhs-inline" },
						...
						h("button", {
							className: "dhs-switch"
								+ (data.retainSessions === true ? " dhs-switch-on" : ""),
							role: "switch",
							"aria-checked": data.retainSessions === true ? "true" : "false",
							title: data.retainSessions === true
						...
						h("span", {
							className: data.retainSessions === true ? "dhs-ok" : "dhs-err",
						}, data.retainSessions === true ? "已开启" : "不主动同步")),
					h("div", { className: "dhs-muted" },
						data.retainSessions === true
							? "AI 每轮答完后自动写入 Hindsight（对新会话生效）。"
							: "默认关闭：不主动写入会话记录；对话中明确要求记住的内容仍会正常入库（被动同步）。")),
